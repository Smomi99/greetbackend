...
// alert(Bubble.bubble_button_config[0]?.color)
    // alert(article.getAttribute('data-bubbleinfo'))
    var NotificationSound=$(".Bubble-large-container").data("notisound")
    $(video1).attr('src','http://localhost:5000/files/users/'+Bubble.user_id+'/Bubble-Videos/'+Bubble.bubble_video)
    if (CheckContrast(Bubble.bubble_border_color)){
    ...
    $(".send-message-container").css('background-color',Bubble.bubble_border_color)
    //----------------------------------//
    var darkenColor=  getTintedColor(Bubble.bubble_button_config[0]?.color,-50)
    $(".multi-step-btn").css('background-color',Bubble.bubble_button_config[0]?.color)
    $(".sender-left-msg").css("background-color",Bubble.bubble_button_config[0]?.color)
     $("#send-msg-btn").css("background-color",Bubble.bubble_button_config[0]?.color)
 $("#fake-send-msg-btn").css("background-color",Bubble.bubble_button_config[0]?.color)
    
    $(".sender-left-msg").css("border-color",Bubble.BubbleButtonColor)

    $(".multi-step-btn").css('border-color',darkenColor)
    if (CheckContrast(Bubble.bubble_button_config[0]?.color)){
...
$("#fake-send-msg-btn").css('color',"#ffffff")
    ...
    if (Bubble.bubble_name=='' ||!Bubble.bubble_name.replace(/\s/g, '').length){
        $(".live-chat-name").text('LIVE CHAT')
        $(".Toast-Name").text('LIVE CHAT')

    }else {
        $(".live-chat-name").text(Bubble.bubble_name)
        $(".Toast-Name").text(Bubble.bubble_name)
...
var Cookies=getCookie("CompleteGreet")
    ...
    var City
    var Country
    var GreetMsg=Bubble.bubble_name
...
// console.log(window.location.href)
    $("#step-2-btn").on('click',function (){
        ...
        if (Name==''|| Email==''){
            ...
            if (!$.trim(Bubble.bubble_name)||Bubble.bubble_name==null){
            ...
            $(".sender-left-msg").css("background-color",Bubble.bubble_button_config[0]?.color)
             $("#send-msg-btn").css("background-color",Bubble.bubble_button_config[0]?.color)
 $("#fake-send-msg-btn").css("background-color",Bubble.bubble_button_config[0]?.color)
            $(".sender-left-msg").css("border-color",Bubble.bubble_button_config[0]?.color)
...
$("#fake-send-msg-btn").css('color',"#ffffff")
            ...
            $.ajax({
                ...
                success: function (data, status) {
                    ...
                    socket.emit('new chat',startChat(Name,Email,Bubble.user_id,chat_ID,Bubble.id,CookieID,ChatCode),IPAddress,City,Country)
                    socket.emit("new connection",ChatCode,Bubble.user_id,Email)
        ...
        var timer=setInterval(function (){
            if (ChatCode==undefined||socket==undefined){
                ...
                socket.on(ChatCode,(msgInfo,chat,ImageURL)=>{
                    ...
                    if(msgInfo.SenderID==Bubble.user_id){
